namespace PM.Editor
{
	...
	public class CustomInspector_UISingleton : Editor
	{
		...
		{
			manusList = new ReorderableList(serializedObject, serializedObject.FindProperty("manusSelectables"), true,
				true, true, true);

			manusList.drawHeaderCallback += (Rect rect) =>
			{
				EditorGUI.LabelField(new Rect(rect.x, rect.y, rect.width / 2, rect.height),
					manusList.serializedProperty.displayName, EditorStyles.boldLabel);
				EditorGUI.LabelField(new Rect(rect.x + rect.width / 2 + 8, rect.y, rect.width / 2 - 8, rect.height),
					"Name(s)", EditorStyles.boldLabel);
			};

			manusList.elementHeight = EditorGUIUtility.singleLineHeight + 12;

			manusList.elementHeightCallback += (int index) =>
				manusList.serializedProperty.GetArrayElementAtIndex(index).FindPropertyRelative("names")
					.arraySize * (EditorGUIUtility.singleLineHeight + 2) + 10;

			manusList.drawElementCallback += (Rect rect, int index, bool isActive, bool isFocused) =>
			{
				var selProp = manusList.serializedProperty.GetArrayElementAtIndex(index)
					.FindPropertyRelative("selectable");
				var namesProp = manusList.serializedProperty.GetArrayElementAtIndex(index)
					.FindPropertyRelative("names");
				...
				{
					var nameRect = new Rect(selRect.xMax + 4, rect.y + (EditorGUIUtility.singleLineHeight + 2) * i,
						rect.width / 2 - 2, EditorGUIUtility.singleLineHeight);
